...
<div class="card">
  ...
  <h5 class="card-header info-color white-text text-center py-4">
    <strong style="color: black;">Update NoticeBoard</strong>
  </h5>
  ...
  elseif ($size>1000000) {
   // echo "File is too learge";
    ...
    if(move_uploaded_file($file, $destination)){
     $query= "INSERT INTO noticeboard(classid,session,size,noticetitle,Date) VALUES('$cid','$sess','$size','$filename','$date')";
     ...
     if($data){
       ...
       echo "<script type='text/javascript'>alert('Notice Update unsuccessfull!')</script>";
     }
   }
 }
}
...
if (isset($_GET['file_id'])) {
 ...
 $filepath= 'files/' . $file['noticetitle'];
 ...
 if(file_exists($filepath)){
  ...
  header('Content-Length:' . filesize('files/'.$file['noticetitle']));

  readfile('/files'.$file['noticetitle']);
